...
自作ファイルの役割をパスから判断でき、複数人が同じ場所へ迷わず追加できる構成にします。Unityの参照を壊さないため、アセットを移動するときは対応する `.meta` ファイルも必ず一緒に移動します。
...
`Library/`、`Temp/`、`Logs/`、`UserSettings/`はUnityが各PCで生成するため、リポジトリには含めません。
...
第三者アセットのフォルダ名と内部構造は、再導入や更新をしやすくするため原則として変更しません。
...
Asset Storeなどから導入したフォルダは、配布元から更新しやすくするため名前と内部構造を維持します。サンプルシーンや配布資料も、削除判断が終わるまでは各アセット内に残します。
...
新しいC#ファイル名は、原則として中の主要クラス名と一致させます。曖昧な `Manager` フォルダには集約せず、管理対象の役割に応じて配置します。
...
次のファイルは調査時点で本編シーン、Prefab、マテリアル、コードから参照されていません。将来用・デバッグ用・配布元の原本である可能性があるため、今回の整理では削除していません。
...
コードの処理内容、シーンのオブジェクト、Prefabの内容、設定値は変更していません。
